...
## How to Detect and Correct Duplicates

Visual Layer supports multiple workflows to help eliminate redundancy:

- **Detect Duplicates:**  
  Go to **"Add Filter"** → **"Duplicates"** → select **"IS"** as the logic operator → set the desired confidence threshold (default is 1).  
  Export the results using **"Matching the applied filter."**

- **Correct Duplicates:**  
  Go to **"Add Filter"** → **"Duplicates"** → select **"IS NOT"** as the logic operator → set the desired confidence threshold (default is 1).  
  Export the results using **"Matching the applied filter."**
