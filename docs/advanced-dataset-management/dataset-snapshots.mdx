...
- Snapshots require the dataset to be in **Ready** status for create, restore, and clone operations.
- Only one snapshot operation (create, restore, or clone) can run at a time per dataset. The three-dot menu actions are disabled on other snapshots while an operation is in progress.
- Restore and clone operations put the dataset in **Read Only** status, blocking other data-modifying tasks until the operation completes.
