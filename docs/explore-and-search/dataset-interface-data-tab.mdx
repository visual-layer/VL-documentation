...
- [Enrichment jobs](/docs/advanced-dataset-management/enrich-overview) triggered from the [Enrich tab](/docs/explore-and-search/dataset-interface-enrich-tab)
- [Export](/docs/advanced-dataset-management/export-dataset) and [share](/docs/collab-and-downstream/share-dataset) actions

Use the activity log as a quick answer to "what happened to this dataset and when?" For long-running operations still in flight, the [Task Manager](/docs/advanced-features/task-manager) shows live status across every dataset. For a point-in-time copy of the dataset, see [Dataset Snapshots](/docs/advanced-dataset-management/dataset-snapshots).
