...
gsutil ls gs://visual_db/

# Copy files from bucket to local machine
gsutil cp gs://visual_db/file.zip ./

# Copy files from local machine to bucket
gsutil cp ./local-file.txt gs://visual_db/

# Sync directories
gsutil rsync -r ./local-folder gs://visual_db/remote-folder
```
...
Set up application default credentials (this matches your Google login from Step 2):
