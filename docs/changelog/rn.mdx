...
We are excited to present the latest release of Visual Layer. This update introduces powerful new capabilities for 3D model support, enhanced rendering performance, and improved stability across various platforms.

## New Features & Enhancements

### glTF 2.0 3D Model Support

Visual Layer now supports importing and rendering 3D models that adhere to the glTF 2.0 standard. This includes full support for Physically-Based Rendering (PBR) materials, enabling more realistic and visually consistent assets within the visual layer. This enhancement opens up new possibilities for working with complex 3D datasets and visualizations.

### Scene Graph API

A new programmatic interface is now available for manipulating objects in the 3D scene at runtime. This Scene Graph API allows for dynamic scenes where object properties such as position, rotation, and scale can be modified via script, providing greater interactive capabilities and enabling more sophisticated visual analysis workflows.

### Optimized Texture Compression

We've implemented Adaptive Scalable Texture Compression (ASTC) for textures on supported hardware. This optimization reduces VRAM usage and decreases initial load times, resulting in improved performance and a more responsive user experience when working with texture-heavy datasets.

### Enhanced Raycasting Precision

The raycasting system now uses a more precise algorithm for intersection tests against complex meshes. This improvement enhances the accuracy of object selection and interaction in detailed scenes, making it easier to work with intricate 3D models and datasets.

## Fixed Issues

### Rendering Stability Improvements

* **Resolved rendering artifacts on Android devices:** Corrected a shader compilation error that caused visual glitches, such as flickering and incorrect colors, on devices utilizing Mali GPUs. This ensures consistent and correct rendering across a wider range of Android hardware.

* **Fixed memory leak in asset loader:** Addressed a memory leak that occurred when repeatedly loading and unloading models. The asset loader now properly deallocates all associated resources, improving application stability during long sessions.

* **Corrected alpha blending on transparent objects:** Adjusted the rendering queue and depth sorting for transparent materials. Objects now render with the correct alpha blending, resolving issues where background objects would incorrectly appear in front of transparent surfaces.
