...
We are excited to present the latest release of Visual Layer. This update introduces powerful new capabilities for 3D content rendering and significant performance improvements for handling large-scale visual datasets.

## New Features & Enhancements

### 3D Model Support with glTF 2.0 Format

Visual Layer now supports the import and rendering of 3D models using the industry-standard glTF 2.0 format. This comprehensive implementation includes:
...
This update enables more efficient transmission and loading of 3D content, opening new possibilities for visual dataset analysis in three-dimensional space.
...
* **Frustum culling**: Prevents rendering of objects outside the camera's view
* **Occlusion culling**: Skips rendering of objects hidden behind other geometry
* Significant frame rate improvements for complex scenes with high object counts

These optimizations ensure smooth navigation and interaction even with the most demanding visual datasets.

### Custom Font Support

Users can now upload and apply custom fonts to projects, enabling greater brand consistency and design flexibility:

* Support for `.woff`, `.woff2`, and `.ttf` font file formats
* New API endpoint for font management
* Integration with the theme editor's font dropdown menu

### Improved Dataset Rendering Performance
...
* **Resolved texture caching memory leak**: Fixed an issue where the texture caching system failed to deallocate VRAM for unused textures, preventing progressive memory consumption during runtime
* **Stable memory usage**: Unused textures are now correctly released, leading to improved application stability

### Mobile UI Fixes

* **Corrected header component alignment**: Fixed CSS alignment issues where logo and navigation links appeared misaligned on viewports narrower than 768px
* **Improved mobile responsiveness**: All header elements now maintain proper vertical alignment across mobile devices

We've also addressed various other bugs and made improvements to enhance your overall experience with Visual Layer.
</Update>
