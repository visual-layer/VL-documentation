...
# Visual Layer v2.5.0 - August 10, 2025

This release introduces a new dark mode theme, significant performance improvements for data tables, and key bug fixes.

### New Features

*   **VIS-101: Dark Mode**
    *   A new system-wide dark mode theme is now available. This theme can be enabled in user settings to reduce eye strain and improve visual ergonomics in low-light conditions. The theme applies to all UI components, including the main interface, modals, and data tables.

### Improvements

*   **VIS-102: Table Rendering Performance**
    *   The primary data table component has been refactored to use virtualization. This change significantly improves UI performance when rendering large datasets, with tables containing over 10,000 rows now loading up to 50% faster.
*   **VIS-105: API Key Generation Workflow**
    *   A "Copy to Clipboard" button has been added to the API key generation modal. This facilitates a more efficient and error-resistant workflow by eliminating the need for manual text selection and copying.

### Fixes

*   **VIS-103: Unresponsive Login Button in Safari**
    *   Resolved a CSS rendering issue where the login button was unresponsive in Safari browsers. The fix corrects the z-index stacking order, ensuring the button is always accessible.
</Update>
