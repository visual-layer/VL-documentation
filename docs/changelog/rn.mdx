...
This update brings new collaboration tools and enterprise-grade security features, along with important stability improvements.

## New Features & Enhancements

- **[VL-1993] SAML Single Sign-On (SSO):** SAML-based single sign-on (SSO) is now available for enterprise customers. This allows users to authenticate with their own identity provider, improving security and simplifying user management. The integration supports the SAML 2.0 protocol and has been tested with major identity providers, including Okta, Azure AD, and Google Workspace. To enable SAML SSO, please contact our support team.
- **[VL-2001] Data Point Commenting:** We have introduced a new commenting feature that allows users to leave comments on individual data points in a dataset. This is useful for collaboration and for leaving notes for yourself or for other team members. Comments are displayed in a new "Comments" tab in the right panel. You can also see a summary of all comments in the dataset in the "Insights" tab.

## Fixed Issues

- **[VL-2010]** Fixed a bug that caused the application to crash when exporting a large dataset to CSV. The export process has been optimized to handle large datasets more efficiently.
- **[VL-2012]** Resolved an issue where the "Last Modified" date was not being updated correctly. The date is now updated whenever a change is made to the dataset.

## Known Issues

- **[VL-2015]** We are aware of an issue where comments are not always displayed in real-time. We are working on a fix for this and will release it in a future update.
</Update>
<Update label="2025-08-11" >
This release introduces a highly requested dark mode feature and adds support for Okta single sign-on (SSO) to enhance user experience and security. We've also addressed several bugs to improve stability.

## New Features & Enhancements

- **[VL-4890] Dark Mode:** Added a new "dark mode" to the user interface. This can be enabled in the user settings and will apply a dark theme to the entire application. This is intended to reduce eye strain in low-light environments.
- **[VL-4891] Okta Single Sign-On (SSO):** The application now supports single sign-on (SSO) with Okta. This allows users to log in with their Okta credentials, which simplifies the login process and improves security.

## Fixed Issues

- **[VL-4892]** Fixed a bug where the application would crash when uploading a file with a name containing special characters. The application now correctly handles all special characters in file names.
- **[VL-4893]** Resolved an issue where the user's session would time out after 5 minutes of inactivity. The session timeout has been increased to 30 minutes.

## Known Issues

- **[VL-4894]** When using the new dark mode, some of the text in the application may be difficult to read. We are working on a fix for this and will release it in a future update.
- **[VL-4895]** Some users have reported issues with the Okta SSO integration. We are investigating these issues and will provide an update as soon as possible.
</Update>
