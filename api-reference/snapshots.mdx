...
| `is_current` | boolean | `true` if this snapshot represents the dataset's active state. |
...
Track the restore progress using the [Task Manager API](/api-reference/task-manager). A `SNAPSHOT_RESTORE` task appears with the dataset's ID. The dataset returns to **Ready** status once the restore completes.
