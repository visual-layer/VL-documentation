...
| Phase | `status_new` | What's Happening |
|-------|--------------|------------------|
| Before | `READY` | Dataset is idle, ready for operations. |
| Processing | `READ ONLY` | New media is being processed. Browsing still works. |
| Awaiting Reindex | `PARTIAL INDEX` | New media is visible but the dataset has not been re-clustered. |
| Reindexing | `READ ONLY` → `INDEXING` | Re-clustering in progress. |
| Complete | `READY` | All media processed and clustered. |
