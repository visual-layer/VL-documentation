...
<Card title="How This Helps" icon="plus">
...
| Phase | `status` | `status_new` | What's Happening |
|-------|----------|--------------|------------------|
| Before | `READY` | `READY` | Dataset is idle, ready for operations |
| Processing | `UPDATING` | `READ ONLY` | New media is being processed (browsing still works) |
| Awaiting Reindex | `PENDING_INDEX` | `PARTIAL INDEX` | New media is visible but dataset is not re-clustered |
| Reindexing | `INDEXING` | `INDEXING` | Full re-clustering in progress |
| Complete | `READY` | `READY` | All media processed and clustered |
